...
procedure Simulation is
   ...
   Car_name : constant array (Product_Type) of String (1 .. 9) := ("SUV      ", "Limousine", "Sedan    ", "Combi    "); -- different car types
   ...
   begin
      ...
      loop
         ...
         Put_Line("L: Produced " & Car_name(Number_of_line) & " number "  & Integer'Image(Number_of_car));         
    ...
    select
         accept Take(Car: in Product_Type; Number: in Integer; If_taken: out Boolean) do
            ...
            if May_take(Car) then
               Put_Line("B: " & Car_name(Car) & " accepted, nr " & Integer'Image(Number));
               Warehouse(Car) := Warehouse(Car) + 1;
               In_Warehouse := In_Warehouse + 1;
               If_taken := True;
            else
               Put_Line("B: " & Car_name(Car) & " rejected, nr " & Integer'Image(Number)); 
               If_taken := False;
            end if;

            exception
               when exceptioon => Put_Line("BUFFER: Car not accepted, buffer is full!");
