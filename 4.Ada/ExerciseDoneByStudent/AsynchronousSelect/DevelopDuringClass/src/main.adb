...
procedure main is
   ...
   subtype Quality is Integer range 1 .. 9;

   subtype Product_Type is Integer range 1 .. Number_Of_Products;
   subtype Assembly_Type is Integer range 1 .. Number_Of_Assemblies;
   subtype Consumer_Type is Integer range 1 .. Number_Of_Consumers;
      
   Product_Name: constant array (Product_Type) of String(1 .. 8):= ("Mango   ", "Cocoa   ", "Milk    ", "Cherry  ", "Water   ");
   Assembly_Name: constant array (Assembly_Type) of String(1 .. 9):= ("Chocolate", "Cherry   ", "Mango    ");
   
   package Random_Quality is new Ada.Numerics.Discrete_Random(Quality);
   package Random_Assembly is new Ada.Numerics.Discrete_Random(Assembly_Type);
      
   type My_Str is new String(1 ..256);

   -- Producer produces determined product
   task type ShakeProducer is
   ...
   task type MilkShake is
      -- Accept a product to the storage provided there is a room for it
      entry Take(Product: in Product_Type; Number: in Integer; Took: out Integer);
      -- Deliver an assembly provided there are enough products for it
      entry Deliver(Assembly: in Assembly_Type; Number: out Integer; Took: out Integer);
   end MilkShake;

   P: array ( 1 .. Number_Of_Products ) of ShakeProducer;
   K: array ( 1 .. Number_Of_Consumers ) of ShakeConsumer;
   B: Milkshake;

   task body ShakeProducer is
   ...
   begin
      ...
      loop
         ...
         Put_Line("Produced product " & Product_Name(Product_Type_Number) & " number "  & Integer'Image(Product_Number));
         -- Accept for storage
         
         loop
            select
               delay 3.0;
               Put_Line("Sorry, you must wait");

            then abort
               B.Take(Product_Type_Number, Product_Number, Took);
      ...
      begin
         ...
         loop
            ...
            begin
               ...
               if Random_Quality.Random(G1) < 5 then
                  raise Quality_Failure with "Too bad quality of products to produce an assembly";
               end if;
   
               loop
                  select
                     delay 3.0;
                     Put_Line("Please wait until we will have your product");
                  
                  then abort
                     B.Deliver(Assembly_Type, Assembly_Number, Took);
   
                     if Took = 1 then
                        Put_Line(Consumer_Name(Consumer_Nb) & ": taken shake " &
            ...
            exception
               when Fail:Quality_Failure => Put_Line(Exception_Message(Fail));
            end;
         end loop;
      end ShakeConsumer;
   ...
   task body MilkShake is
      ...
      begin
         if In_Storage >= Storage_Capacity then
            for W of Storage loop
               In_Storage := In_Storage - (W/2);
               W := W - (W/2);
            end loop;
            
            -- [return False;] [in the other version there is this istruction]
         end if;
         ...
         if Integer'Max(0, Max_Assembly_Content(Product) - Storage(Product)) > 0 then
            return True;              -- exactly this product lacks
         end if;
   ...
   begin
      Put_Line("Milkshake started");
      Setup_Variables;

      loop
         select
            accept Take(Product: in Product_Type; Number: in Integer; Took: out Integer) do
         ...
         or
            ...
            accept Deliver(Assembly: in Assembly_Type; Number: out Integer; Took: out Integer) do
               if Can_Deliver(Assembly) then
                  Put_Line("Delivered assembly " & Assembly_Name(Assembly) & " number " & Integer'Image(Assembly_Number(Assembly)));
                  ...
                  Put_Line("Lacking products for shake " & Assembly_Name(Assembly));
                  Number := 0;
                  Took := 0;
               end if;
            end Deliver;
