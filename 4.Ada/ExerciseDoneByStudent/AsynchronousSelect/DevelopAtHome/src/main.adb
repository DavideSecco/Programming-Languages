...
procedure main is
   ...
   begin
      ...
      loop
         ...
         -- [begin (c'è perchè sopra ha aggiunto l'eccezione)]
         delay Duration(Random_Consumption.Random(G)); --  simulate consumption
         Assembly_Type := Random_Assembly.Random(G2);
         -- take an assembly for consumption

         -- [if regarding Random_Quality]
   ...
   task body MilkShake is
      ...
      begin
         if In_Storage >= Storage_Capacity then
            for W in Storage loop
               In_Storage := In_Storage - (W/2);
               W := W - (W/2);
            end loop;
            -- [ATTENZIONE!!!! NELL'ALTRO NON C'È RETURN FALSE]
            return False;
	      end if;
         ...
         if Free >= Lacking_room then
            return True;           -- there is enough room in storage for arbitrary assembly
         else
            return False;     -- no room for this product
         end if;
      end Can_Accept;
